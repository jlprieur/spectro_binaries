...
C  Scindé en sous-programmes en mai 2005
C
C JLP
C Version 11/04/2006
C************************************************************************

C***********************************************************************
C Sous-programme principal de traitement des vitesses radiales 
C de systèmes binaires à une seule composante détectée (BS1) 
C
C INPUT:
C PERIOD: période (en jours), utilisée seulement si mode automatique
C OBJET: nom de l'objet. Si égal à "mode-automatique", le programme 
C        bascule en mode automatique de recherche de période. 
C
C OUTPUT:
C SIGMA: écart-type moyen
C***********************************************************************
      SUBROUTINE BS1_PROCESS(OBJECT,PERIOD,SIGMA)
      IMPLICIT NONE
      INTEGER IDIM
      PARAMETER (IDIM=120)
      ...
      INTEGER NORB,NN,NITER,I,IOPT,STATUS
      REAL EE,OMEGA,KA,VZERO,PI,PERIOD,SIGMA
      REAL TZERO,ASINI,FDEM,ERR_ASINI,ERR_FDEM
      DOUBLE PRECISION PP,T
      CHARACTER OBJECT*40,OUT_RESULTS*60,OUT_LATEX*60
      CHARACTER BUFFER*80
...
10    FORMAT(A)
      ...
      INTERACTIVE=(OBJECT(1:16).NE.'MODE-AUTOMATIQUE')
     1            .AND.(OBJECT(1:16).NE.'mode-automatique')
...
C Mode automatique de recherche de période
C Test sur la valeur de PERIOD:
...
C Minimisation par moindres carrés:
     ...
     1                   STATUS)
C In case of error, output a large value for SIGMA and return at once:
      IF(STATUS.NE.0)THEN
       SIGMA = 1000.
       IF(INTERACTIVE)THEN 
        WRITE(6,*) "BS1_PROCESS/Error while inverting matrix"
        WRITE(LU_OUT_TXT,*)"BS1_PROCESS/Error while inverting matrix"
       ENDIF
       CLOSE(LU_OUT_TXT)
       RETURN
      ENDIF
...
C Visualisation des courbes:
      ...
      IF(IOPT.EQ.1)THEN
C Vitesse/Phase
        CALL JLP_PLOT_BS1(TOBS,VIT,POI,T,PP,NN,EE,OMEGA,KA,VZERO,
     1                    OBJECT,PUBLI) 
...
C*********************************************************************
      SUBROUTINE JLP_PLOT_BS1(TOBS,VIT,POI,T,PP,NN,EE,OMEGA,KA,
     1                        VZERO,OBJECT,PUBLI) 
      IMPLICIT NONE
      INTEGER IDIM,KCUR
      PARAMETER (IDIM=1024,KCUR=3)
      REAL TOBS(*),VIT(*),POI(*),PHI_MINI
      DOUBLE PRECISION T,PP,PHI
      REAL PI,EE,OMEGA,KA,VZERO,VIC,VV
      INTEGER ITER,I,I1,I2,NN,KK,NPTS(KCUR)
      REAL XPLOT(IDIM,KCUR),YPLOT(IDIM,KCUR)
      CHARACTER CHAR1*30,CHAR2*30,CHAR3*40,OBJECT*40
...
C to display the peaks of highly eccentric orbits in one piece: 
      ...
      NPTS(1)=I1
      NPTS(2)=I2
C Cercles pleins (primaire, mesures fiables)
      NCHAR(1)='913'
C Triangles vides (primaire, mesures moins fiables)
      NCHAR(2)='213'
C     0: histogram
C     1: a tiny point
C     2: white triangle (base down)
C     3: black triangle (base down)
C     4: cross +
C     5: cross X
C     6: open square
C     7: filled square
C     8: open circle with a diameter (20 * size)
C     9: filled circle with a diameter (20 * isize)
C     12: white triangle (base up)
C     13: black triangle (base up)
...
C************************************************************************
      SUBROUTINE MAT_INV(A,D,NP,INTERACTIVE,STATUS)
      IMPLICIT NONE
      INTEGER NP,STATUS
...
C permutation dans A, sans le faire aussi dans B
      IF(A(K,K).EQ.0)THEN
        if(INTERACTIVE) write(6,*)' Pivot nul pour K=',K
   ...
   12   CONTINUE
C JLP2006:
C Retour si on n'a pas trouvé de terme non nul
        IF(L1.EQ.-1)THEN
           STATUS=-1
           RETURN
        ENDIF
...
C*****************************************************************
     ...
     1                        TOBS,VIT,RES,POI,NN,OUT_LATEX,OBJECT)
      ...
      CHARACTER OUT_LATEX*40,OBJECT*40
      INTEGER I,LU_OUT
      LU_OUT=8
...
C*************************************************************************
     ...
     1                   STATUS)
      ...
      INTEGER NN,NITER,I,J,K,ITER,STATUS
...
C en sortie: D=matrice inverse
      CALL MAT_INV(A,D,NP,INTERACTIVE,STATUS)
      IF(STATUS.NE.0)THEN
        RETURN
      ENDIF
...
C Calcule les nouveaux elements en ajoutant la correction
C calculée (méthode de Newton-Raphson avec paramètre de relaxation égal à 1:
      DIST=0
      F=0
      DO I=1,6
        DELTA(I)=0
...
C JLP2006: it is Newton's method ("tangent-method") 
c or Newton-Raphson method with relation factor equal to 1.0 ...
C So I correct the old values of the parameters by adding 1.0 * CORREC()
      PP=PP+CORR(1)
...
C Erreur grave 
        WRITE(6,2000)
 2000   FORMAT(5X,'BS1_FIT/FATAL ERROR/ ABS(EE).GE.1,',
     1         ' POURSUITE DU CALCUL IMPOSSIBLE')
        IF(INTERACTIVE)THEN
